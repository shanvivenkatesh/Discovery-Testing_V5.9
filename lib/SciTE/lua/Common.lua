...
function Common:LastWord(line)
	...
	from, to =  word_found:find(";")
	while from ~= nil do
--~ 		print(from .. " type:" .. editor.StyleAt[from])
		if editor.StyleAt[from] == SCE_AU3_COMMENT then
			word_found = string.sub (word_found, 1 , from-1)   -- remove comment
			from = nil                             -- exit loop
		else
			from, to =  word_found:find(";",from+1)      -- find next ; as this one is not a comment
		end
	end
	-- get the last work of the line
	for w in string.gfind(word_found, "%a+") do
		last_word = w
	end
