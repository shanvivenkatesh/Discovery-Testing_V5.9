...
function AutoItIndentFix:OnUpdateUI()
	...
	if self.Check_Indent and not editor:AutoCActive() then
		...
		local Curr_foldLvl = math.mod(Curr_lev, 4096)
		local Prev_foldLvl = math.mod(Prev_lev, 4096)
		local Next_foldLvl = math.mod(Next_lev, 4096)
		local Parent_line =  editor.FoldParent[line]
		local Parent_foldLvl = math.mod(Parent_line, 4096)
		local Prev_Parent_line =  editor.FoldParent[line-1]
		local Prev_Parent_foldLvl = math.mod(Parent_line-1, 4096)
		local Next_Parent_line =  editor.FoldParent[line+1]
		local Next_Parent_foldLvl = math.mod(Parent_line+1, 4096)
		local Curr_IsFoldHeader = math.mod(math.floor(Curr_lev / 8192), 2) == 1
		local Prev_IsFoldHeader = math.mod(math.floor(Prev_lev / 8192), 2) == 1
		local Next_IsFoldHeader = math.mod(math.floor(Next_lev / 8192), 2) == 1
		...
		if fold_lastword == "then" then
			...
			Parent_foldLvl = math.mod(Parent_line, 4096)
