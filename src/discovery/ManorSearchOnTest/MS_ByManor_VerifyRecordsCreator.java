package discovery.ManorSearchOnTest;
...
public class MS_ByManor_VerifyRecordsCreator extends Wrapper_methods{
	...
	public void ByMonor_VerifyRecordsCreator() throws IOException, InterruptedException{
	//launch the browser and url	
	launchDriver("http://test.discovery.nationalarchives.gov.uk/manor-search", "chrome");
	Thread.sleep(3000);
	//Enter the wildcard
	...
	//Assert.assertTrue(total_RecordsCreators.contains("Record creators 18,055"));
	
	if (total_RecordsCreators.matches("Record creators 18,[0-9].*"))
